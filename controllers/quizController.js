...
import Quiz from "../models/quizModel.js";
import { createLog } from "./activityLogController.js"; // import the logging function
...
export const deleteQuestion = async (req, res) => {
  try {
    ...
    await createLog({
      ...
      quizId: quiz._id,
      questionIndex: index,
      details: { questionText: deletedQuestion.text }
    });
...
export const updateQuestion = async (req, res) => {
  try {
    ...
    await createLog({
      ...
      quizId: quiz._id,
      questionIndex: index,
      details: { oldQuestionText: oldQuestion.text, newQuestionText: text }
    });
...
export const updateQuiz = async (req, res) => {
  try {
    const { title, description, category, difficulty, timeLimit, questions } = req.body;
    const oldQuiz = await Quiz.findById(req.params.quizId);
    ...
    await createLog({
      ...
      details: { oldTitle: oldQuiz.title, newTitle: updatedQuiz.title }
    });
